...
create view view_compras as select*from	compra;
select*from view_compras;
select descProduto, dataCompra from view_compras;

create view view_produtos as select*from produto;
select*from view_produtos;
select codigoProduto, codFornecedor from view_produtos;

create view view_fornecedor as select*from fornecedor;
select*from view_fornecedor;
select nomeFornecedor, enderecoFornecedor from view_fornecedor;

drop view view_produtos;
create view view_compras as select 
c.descProduto as compra, f.nomeFornecedor 
from compra as c join fornecedor as f 
on c.codProduto = f.codigProduto;  
select*from view_compras;

create view view_produtos as select 
p.codigoProduto as produto, f.enderecoFornecedor 
from produto as p join fornecedor as f 
on p.codigoProduto = f.codigProduto;  
select*from view_compras;
...
select nomeCliente, telCliente from view_cliente;

create view view_produtos as select*from produto;
select*from view_produtos;
select descProduto, valorUnitario from view_produtos;

create view view_recibo as select*from recibo;
select*from view_recibo;
select vTotalCompra, serie from view_recibo;

drop view view_clientes;
create view view_clientes as select 
r.vTotalQtd as recibo, c.nomeCliente
from recibo as r join cliente as c 
on r.codCliente = c.codigoCliente;  
...
create view view_aluno as select*from aluno;
select*from view_aluno;
select nomeAluno, numCurso from view_aluno;

create view view_curso as select*from curso;
select*from view_curso;
select nomeCurso, inicioCurso from view_curso;

create view view_professor as select*from professor;
select*from view_professor;
select nomeProfessor from view_professor;

drop view view_professores;
create view view_professores as select 
p.nomeProfessor as professor, mp.materiaProfessor
from professor as p join materiaProfessor as mp 
on p.numProfessor = mp.numerProfessor;  
select*from view_professores;

create view view_cursos as select 
c.nomeCurso as curso, a.nomeAluno 
from curso as c join aluno as a 
on c.numeroCurso = a.numCurso;  
